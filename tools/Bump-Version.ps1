...
if ($changelogContent -match '(?m)^## \[Unreleased\]') {
    ...
    if ($PSCmdlet.ShouldProcess($changelogPath, "CHANGELOG insert ## [$NewVersion] — $today")) {
        $insertMarker = "`n## ["
        $insertAt = $changelogContent.IndexOf($insertMarker)
        if ($insertAt -ge 0) {
            $newEntry = "`n## [$NewVersion] — $today`n`n### Added`n- $ReleaseNotes`n"
            $newContent = $changelogContent.Insert($insertAt, $newEntry)
