...
.DESCRIPTION
    ...
    - ARM SKU restriction status (OK, LIMITED, ZONE-LIMITED, RESTRICTED)
...
.PARAMETER Recommend
    Find alternatives for a target SKU that may be unavailable or zone-limited.
    ...
    foreach ($entry in $lifecycleEntries) {
        ...
        if ($recOutput) {
            ...
            if ($deployedRegion) {
                ...
                $hasRestrictionIssues = @($targetAvail | Where-Object { $_.Status -notin 'OK','LIMITED' }).Count -gt 0
            }
            ...
            if ($hasRestrictionIssues) { $riskReasons.Add("SKU restriction$(if ($deployedRegion) { " ($deployedRegion)" } else { '' })"); $riskLevel = 'High' }
            ...
            $shouldFlagNoAlternatives = $noAlternatives -and ($isOldGen -or $hasRestrictionIssues -or $hasRetirement -or $notAvailForNew)

            # Current-gen (v4+) SKUs with quota as the only risk → recommend quota increase, not SKU change
            $isQuotaOnlyCurrentGen = (-not $isOldGen) -and (-not $hasRestrictionIssues) -and (-not $hasRetirement) -and (-not $notAvailForNew) -and $quotaInsufficient
                            ...
                            if ($rawUpgradeSku) {
                                ...
                                $upgradeRecs.Add([pscustomobject]@{
                                    Rec = [pscustomobject]@{
                                        ...
                                        family   = Get-SkuFamily $mappedSku
                                        score    = $upScore
                                        RestrictionStatus = $upRestrictions.Status
                                        IOPS     = $upIOPS
                                        MaxDisks = $upMaxDisks
                                        priceMo  = $upPriceMo
            ...
            if ($selectedRecs.Count -eq 0) {
                ...
                foreach ($sel in $selectedRecs) {
                    ...
                    if ($sel.MatchType -like 'Upgrade:*' -and $upgradePathData) {
                        ...
                        if ($rec.restrictionStatus -eq 'Not scanned') {
                    ...
                    $lifecycleResults.Add([pscustomobject]@{
                        ...
                        AltRestrictionStatus      = $rec.restrictionStatus
        ...
        foreach ($family in ($familyGroups.Keys | Sort-Object)) {
            ...
            foreach ($sku in $skus) {
                ...
                $detailObj = [pscustomobject]@{
                    ...
                    RestrictionStatus     = [string]$skuRestrictions.Status
        ...
        if ($rows.Count -gt 0) {
            ...
            foreach ($row in $rows) {
                ...
                $color = switch ($row.Status) {
                    'OK' { 'Green' }
                    { $_ -match 'LIMITED|ZONE-LIMITED' } { 'Yellow' }
        ...
        foreach ($fam in $SelectedFamilyFilter) {
            ...
            if ($detailRows.Count -gt 0) {
                ...
                foreach ($regionGroup in $regionGroups) {
                    ...
                    $dColWidths = [ordered]@{ SKU = 26; vCPU = 5; MemGiB = 6; Gen = 5; Arch = 5; ZoneStatus = 22; Status = 18; Avail = 8 }
                    if ($ShowPlacement -and $SkuFilter -and $SkuFilter.Count -gt 0) {
                        $dColWidths['Alloc'] = 8
                    }
                    if ($FetchPricing) {
                    ...
                    foreach ($dr in $regionRows) {
                        ...
                        $color = switch ($dr.RestrictionStatus) {
...
foreach ($family in ($allFamilyStats.Keys | Sort-Object)) {
    ...
    foreach ($regionItem in $allRegions) {
        ...
        if ($regionStats) {
            $status = $regionStats.RestrictionStatus
...
Write-Host ("  $($Icons.ZONE_LIMITED)".PadRight(16) + "= Some zones returned ARM restriction records; verify placement/deployment.") -ForegroundColor Yellow
...
foreach ($family in $allFamilyStats.Keys) {
    ...
    foreach ($regionKey in $stats.Regions.Keys) {
        ...
        if ($stats.Regions[$regionKey].RestrictionStatus -eq 'OK') {
...
if ($hasBest) {
    ...
    foreach ($family in ($allFamilyStats.Keys | Sort-Object | Select-Object -First 5)) {
        ...
        if ($bestRegion) {
            ...
            Write-Host " ($($regionStat.RestrictionStatus))" -ForegroundColor DarkYellow
        }
    }
}
...
foreach ($family in ($allFamilyStats.Keys | Sort-Object)) {
    ...
    foreach ($regionKey in ($stats.Regions.Keys | Sort-Object)) {
        ...
        if ($regionStat) {
            if ($regionStat.RestrictionStatus -eq 'OK') {
                $regionsOK.Add($regionKeyStr)
            }
            elseif ($regionStat.RestrictionStatus -match 'LIMITED|zone-limited|PARTIAL|RESTRICTED|BLOCKED') {
                # Shorten status labels for narrow terminals
                $shortStatus = switch -Regex ($regionStat.RestrictionStatus) {
                    'ZONE-LIMITED' { 'CONSTRAINED' }
                    default { $regionStat.RestrictionStatus }
                }
    ...
    $exportRow = [ordered]@{
        ...
        SKUs_OK    = (($stats.Regions.Values | Where-Object { $_.RestrictionStatus -eq 'OK' } | Measure-Object -Property Available -Sum).Sum)
    }
    foreach ($r in $allRegions) {
        $regionStat = $stats.Regions[$r]
        if ($regionStat) {
            $exportRow["$r`_Status"] = "$($regionStat.RestrictionStatus) ($($regionStat.Available)/$($regionStat.Count))"
        }
        else {
    ...
    if ($useXLSX -and (Test-ImportExcelModule)) {
        ...
        try {
            ...
            for ($col = 4; $col -le $lastCol; $col++) {
                ...
                # zone-limited - Light orange
                Add-ConditionalFormatting -Worksheet $ws -Range $statusRange -RuleType ContainsText -ConditionValue "ZONE" -BackgroundColor $yellowFill -ForegroundColor $yellowText
            ...
            # Legend title - RestrictionStatus column contains ARM restriction status
            ...
            $statusItems = @(
                ...
                @{ Status = "ZONE-LIMITED"; Desc = "Some zones returned ARM restriction records; verify placement/deployment." }
            ...
            foreach ($item in $statusItems) {
                ...
                switch ($item.Status) {
                    ...
                    { $_ -in "LIMITED", "ZONE-LIMITED", "PARTIAL" } {
            ...
            $restrictionCol = $null
            for ($c = 1; $c -le $lastCol; $c++) {
                if ($ws.Cells[1, $c].Value -eq "RestrictionStatus") {
                    $restrictionCol = $c
                    break
                }
            }
            ...
            if ($restrictionCol) {
                $colLetter = ConvertTo-ExcelColumnLetter $restrictionCol
                $restrictionRange = "$colLetter`2:$colLetter$lastRow"

                # OK - Green
                Add-ConditionalFormatting -Worksheet $ws -Range $restrictionRange -RuleType Equal -ConditionValue "OK" -BackgroundColor $greenFill -ForegroundColor $greenText

                # LIMITED - Yellow
                Add-ConditionalFormatting -Worksheet $ws -Range $restrictionRange -RuleType Equal -ConditionValue "LIMITED" -BackgroundColor $yellowFill -ForegroundColor $yellowText

                # zone-limited - Light orange
                Add-ConditionalFormatting -Worksheet $ws -Range $restrictionRange -RuleType ContainsText -ConditionValue "ZONE" -BackgroundColor $yellowFill -ForegroundColor $yellowText

                # PARTIAL - Yellow (mixed zone availability)
                Add-ConditionalFormatting -Worksheet $ws -Range $restrictionRange -RuleType Equal -ConditionValue "PARTIAL" -BackgroundColor $yellowFill -ForegroundColor $yellowText

                # RESTRICTED - Red
                Add-ConditionalFormatting -Worksheet $ws -Range $restrictionRange -RuleType Equal -ConditionValue "RESTRICTED" -BackgroundColor $redFill -ForegroundColor $redText
            }
            ...
            $legendData = @(
                ...
                [PSCustomObject]@{ Category = "SKU RESTRICTION STATUS"; Item = "ZONE-LIMITED"; Description = "Some zones have ARM restrictions; others are clear. Verify placement before deploying." }
                ...
                [PSCustomObject]@{ Category = "DETAILS COLUMNS"; Item = "RestrictionStatus"; Description = "ARM SKU restriction status (will be renamed to RestrictionStatus in a future release)" }
            ...
            for ($row = 2; $row -le $legendLastRow; $row++) {
                ...
                elseif ($itemValue -eq "LIMITED" -or $itemValue -eq "ZONE-LIMITED" -or $itemValue -eq "PARTIAL") {
                    $ws.Cells["B$row"].Style.Fill.PatternType = [OfficeOpenXml.Style.ExcelFillStyle]::Solid
                    $ws.Cells["B$row"].Style.Fill.BackgroundColor.SetColor($yellowFill)
                    $ws.Cells["B$row"].Style.Font.Color.SetColor($yellowText)
                }
