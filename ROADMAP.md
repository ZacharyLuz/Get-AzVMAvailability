...
>
> **v3.0.0:** Terminology correction throughout — the `Capacity` field is now `RestrictionStatus`, the `CAPACITY-CONSTRAINED` status is now `ZONE-LIMITED`, and JSON `schemaVersion` moved to `2.0`. The tool reports ARM restriction metadata and quota headroom, which are deployment-readiness signals rather than live capacity guarantees; the output wording now says so. **Breaking for anyone parsing the old field or status names.** See [CHANGELOG.md](CHANGELOG.md) for details.
>
